...
from openregistry.assets.core.models import Period
...
def rectificationPeriod_document_workflow(self):
    ...
    response = self.app.post_json('/{}/items/{}/documents'.format(asset['id'], item_id),
                                  headers=self.access_header,
                                  params={'data': self.initial_document_data})
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    ...
    response = self.app.patch_json('/{}/items/{}/documents/{}'.format(asset['id'], item_id, doc_id),
                                  headers=self.access_header,
                                  params={'data': self.initial_document_data},
                                  status=403
                                  )
    ...
    response = self.app.put_json('/{}/items/{}/documents/{}'.format(asset['id'], item_id, doc_id),
                                  headers=self.access_header,
                                  params={'data': self.initial_document_data},
                                  status=403
                                  )
